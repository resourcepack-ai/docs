...
**No `hitbox`, `mount` or `p_*` bones are written.** ModelEngine has sensible
defaults for all three, and a hitbox invented from geometry would be worse than
the one it sizes itself. Add them in Blockbench if you need them.
