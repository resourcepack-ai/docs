...
ModelEngine won't start an animation just because the blueprint has one.
Nothing in a `.bbmodel` says *play this*, so a summoned model stands still and
that is not a broken export.

**The triggers you picked in Animate mode don't come across** — on loop, on
right click, on range are our plugin's concept and mean nothing to ModelEngine.
Playback there is driven by MythicMobs, by ModelEngine's API, or by a command
plugin such as [MEGAnim](https://hangar.papermc.io/Nogard/MEGAnim).

Names are snake_cased on the way out, so "Idle Wave" in the editor is
`idle_wave` in the blueprint — that's the string MythicMobs or `/meganim play`
wants, and the `README.txt` lists it per model. `/meganim list` shows what
ModelEngine actually registered, which is the quickest way to tell a conversion
problem from a playback one.
