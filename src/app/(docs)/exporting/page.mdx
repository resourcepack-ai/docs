...
/meg spawn <model>             spawns one to look at
```
...
A model that uses a *vanilla* texture is the one case that needs your
attention: ModelEngine can't read Minecraft's own files, so those faces render
untextured. The `README.txt` names any model this affects — paint or upload the
texture into the pack and export again.
