...
`speedometer: false` — see [vehicles](/rp-engine/vehicles#the-speedometer),
...
The rebuild starts **at once**: somebody who walked in on their own should not
stand about as a default skin waiting out a window that exists for crowds.
Anybody who arrives during that rebuild, or in the five seconds after it, is
covered by a single follow-up rather than costing a rebuild each — so a server
filling up at the start of an evening pays two, not twenty. Only players whose
