...
Turned cubes keep their angle rather than being squared off, so a handrail
collides where the rail is instead of filling the box around it — and a turned
slab is a **ramp**: a vehicle rides up it rather than stopping at it. Stairs
built as stacked cubes are climbed a tread at a time, up to a block per step.
